...
export async function sendVerificationEmail(emailAddress, verificationToken) {
  if (!SENDGRID_API_KEY) {
    console.log('SendGrid not configured - skipping verification email:', emailAddress);
    return { success: false, message: 'SendGrid not configured' };
  }

  const verificationUrl = `${process.env.DOMAIN || 'http://localhost:3000'}/verify-email.html?token=${verificationToken}`;
  ...
  const htmlContent = `
    ...
    <html>
      <head>
        <meta charset="utf-8">
        <title>Verify Your Email - Old Dog Systems</title>
        <style>
          ...
          .button { display: inline-block; background: #38bdf8; color: white; padding: 15px 30px; text-decoration: none; border-radius: 5px; font-weight: bold; margin: 20px 0; }
      ...
      <body>
        <div class="header">
          ...
          <p>Welcome to Old Dog Systems!</p>
        </div>
        
        <div class="content">
          <h2>Complete Your Registration</h2>
          <p>Thank you for joining Old Dog Systems. To complete your account setup and access all features, please verify your email address.</p>
          ...
          <p>If you didn't create an account with Old Dog Systems, please ignore this email.</p>
        </div>
  ...
  try {
    await sgMail.send({
      to: emailAddress,
      from: SENDER_EMAIL,
      subject: 'Verify Your Email - Old Dog Systems',
      html: htmlContent,
      replyTo: ADMIN_EMAIL,
    });
